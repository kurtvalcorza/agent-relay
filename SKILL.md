...
Infer the active role and, when useful, a role sequence from the user's request plus current durable workflow state. The user should normally be able to say "continue" without manually assigning a role.
...
- review/audit/hunt bugs -> `Reviewer`
...
- review and sign off -> `Reviewer -> Verifier`
...
Role inference never grants permissions. `Builder` does not imply write access. `Integrator` does not imply merge authority. `Reviewer` does not imply permission to post public comments. `Executor` does not imply access to credentials or private data.
...
5. Infer the current role and any necessary role sequence.
6. Separate:
...
2. Reproduce existing claims where practical before searching for new defects.
3. Review adversarially for silent fallback, semantic mutation, declaration-only success, stale identity, incomplete error handling, concurrency/interruption behavior, cross-platform assumptions, caller-supplied evidence, provenance gaps, partial readiness evidence, and contradictory requirements/tests.
4. Report findings with severity, failure mode, owning layer, and evidence.
5. Distinguish code defects from infrastructure/environment failures.
6. Do not approve solely because the implementation is extensive or tests were reported by the Builder.

Use [references/evidence-protocol.md](references/evidence-protocol.md).
...
2. Identify environment or requirement differences.
...
8. Record the decision and evidence in the durable substrate.
...
4. Infer the role required now; do not blindly preserve the previous agent's role.
...
The core protocol must not depend on GitHub, Claude, ChatGPT, Codex, or another named agent product. A repository/PR workflow is one substrate adapter.
...
- Did role inference avoid creating permissions?
